App = {
    ...
    render: function () {
        ...
        web3.eth.getCoinbase(function (err, account) {
            if (err === null) {
                App.account = account;
                $("#accountAddress").html("Dirección de la billetera: " + account);
            }
        });
        ...
        App.contracts.Election.deployed().then(function(instance) {
          ...
          if(hasVoted) {
            $('form').hide();
              $("#index-text").html("Inciaste sesión con éxito");
              $("#new-candidate").html("No es posible añadir nuevos candidatos cuando hay una votación en curso.");
            $("#vote-text").html("Votaste al candidato número " + localStorage.getItem("votedForID"));
          }
          loader.hide();
          content.show();
          return electionInstance.usersCount();
            ...
            if (localStorage.getItem("finishElection") === "1") {
                $('form').hide();
                $("#index-text").html("Actualmente no existen votaciones en curso");
                $("#vote-text").html("No existen votaciones en curso");
        ...
        for (var i = 1; i <= usersCount; i++) {
            ...
            if (lidNumber === idNumber) {
                if(lpassword === password)
                {
                    ...
                    prompt("El login falló, intente nuevamente");
                }
